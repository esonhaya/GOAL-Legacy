...
final class WebApplication
{
    ...
    {
        $stats = (array) ($post['season_stats'] ?? []); $form = (array) ($post['recent_form'] ?? []); $performance = (array) ($post['performance'] ?? []); $readiness = (array) ($post['readiness'] ?? []); $impact = (array) (($post['career_impact'] ?? [])['items'] ?? []); $impactText = $impact === [] ? 'No material change recorded' : implode(' · ', array_slice(array_map('strval', $impact), 0, 2)); return '<div class="split-list"><p><span>Match performance</span><strong>' . WebView::e($performance['performance_label'] ?? 'No rating') . '</strong></p><p><span>Recent form</span><strong>' . WebView::e($this->formLabel($form)) . '</strong></p><p><span>Readiness after Match</span><strong>' . WebView::e(CareerLabels::value($readiness['label'] ?? null, 'Ready')) . '</strong></p><p><span>Season average</span><strong>' . WebView::e($this->rating($stats['average_match_rating'] ?? null)) . '</strong></p><p><span>League position</span><strong>' . WebView::e($post['club_position'] ?? 'Not available') . '</strong></p><p><span>Career impact</span><strong>' . WebView::e($impactText) . '</strong></p></div>';
    }
