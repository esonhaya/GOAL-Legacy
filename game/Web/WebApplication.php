...
final class WebApplication
{
    ...
    private function post(array $post, array $query, array &$session): array
    {
        ...
        try {
            return match ($action) {
                ...
                'set_on_pitch_role' => $this->setOnPitchRole($post, $session),
    ...
    private function setOnPitchRole(array $post, array &$session): array
    {
        ...
        $session['web_flash'] = 'On-pitch role preference updated. It changes action tendencies only while you are playing.';
    ...
    private function home(string $saveId, array &$session): array
    {
        ...
        if (($recovery['visible'] ?? false) === true) {
            $return = is_array($recovery['first_match_back'] ?? null) ? $recovery['first_match_back'] : null;
            $recoveryBody = '<div class="stat-grid compact">' . WebView::stat('Phase', CareerLabels::value($recovery['phase'] ?? null)) . WebView::stat('Significance', CareerLabels::value($recovery['significance'] ?? null)) . WebView::stat('Medical end', $recovery['medical_end_date'] ?? 'Recorded') . '</div><p>' . WebView::e($recovery['message'] ?? 'Recovery context is based on the recorded availability state.') . '</p>';
            if ($return !== null) {
                $recoveryBody .= '<p><strong>First Match back:</strong> ' . WebView::e($return['date'] ?? 'Recorded') . ' · ' . WebView::e(implode(', ', array_map('strval', (array) ($return['performance'] ?? [])))) . '</p>';
            }
            $recoveryPanel = WebView::section('RETURN TO PLAY', 'Recorded recovery context', $recoveryBody);
        }
        $disciplinePanel = '';
        if (($discipline['active'] ?? false) === true) {
            $disciplineLines = '';
            foreach ((array) ($discipline['suspensions'] ?? []) as $suspension) {
                if (is_array($suspension)) { $disciplineLines .= '<li><strong>' . WebView::e($suspension['scope_label'] ?? 'Competition') . '</strong> · ' . WebView::e($suspension['reason_label'] ?? 'Disciplinary suspension') . ' · ' . (int) ($suspension['remaining'] ?? 0) . ' applicable Match' . ((int) ($suspension['remaining'] ?? 0) === 1 ? '' : 'es') . ' remaining</li>'; }
            }
            $disciplinePanel = WebView::section('DISCIPLINARY ELIGIBILITY', 'Healthy does not mean eligible', '<p class="result-badge">SUSPENDED</p><ul class="fixture-list">' . $disciplineLines . '</ul><p class="muted">The ban is served by completed applicable fixtures; injury and readiness remain separate systems.</p>');
        }
        ...
        $disciplineStatus = ($discipline['active'] ?? false) === true ? 'SUSPENDED' : 'Eligible';
        $situation = '<div class="split-list"><p><span>Availability</span><strong>' . WebView::e($availabilityText) . '</strong></p><p><span>Disciplinary eligibility</span><strong>' . WebView::e($disciplineStatus) . '</strong></p><p><span>Readiness</span><strong>' . WebView::e($readinessLabel) . '</strong></p><p><span>On-pitch role</span><strong>' . WebView::e($onPitchRole['role_label'] ?? 'Safe default') . '</strong></p><p><span>Training focus</span><strong>' . WebView::e(CareerLabels::value($summary['training_focus'] ?? 'balanced')) . '</strong></p><p><span>Career priority</span><strong>' . WebView::e(CareerLabels::value($summary['priority'] ?? 'balanced')) . '</strong></p><p><span>Contract</span><strong>' . WebView::e($this->contractText($summary['current_contract'] ?? null)) . '</strong></p><p><span>Career outlook</span><strong>' . WebView::e(CareerLabels::value(((array) ($summary['career_outlook'] ?? []))['category'] ?? null, 'Not available')) . '</strong></p><p><span>Balance</span><strong>' . WebView::e($this->money((int) ($finance['balance'] ?? 0))) . '</strong></p><p><span>Lifestyle</span><strong>' . WebView::e($financialContext['label'] ?? 'Starting out') . '</strong></p></div><p class="muted">' . WebView::e($financialContext['description'] ?? '') . '</p>';
        if ($positionFocus !== '') {
        ...
        $careerMemoryPanel = $nextMilestone === null ? '' : WebView::section('CAREER MEMORY', 'Next factual landmark', '<p><strong>Approaching ' . WebView::e($nextMilestone['threshold'] . ' ' . $nextMilestone['label']) . '</strong> · ' . WebView::e($nextMilestone['remaining']) . ' to go.</p><p class="muted">This is a record of the football already played, not a quest or reward.</p>');
        ...
        if (($direction['actionable'] ?? false) === true || in_array($attachment['state'] ?? '', ['strong_connection', 'club_figure'], true)) {
            $directionReasons = implode(' · ', array_map('strval', (array) ($direction['reasons'] ?? [])));
            $careerDirectionBody = '<div class="stat-grid compact">'
                . WebView::stat('Direction', $direction['label'] ?? 'Developing')
                . WebView::stat('Club connection', $attachment['label'] ?? 'No current Club')
                . '</div><p>' . WebView::e($direction['description'] ?? '') . '</p>'
    ...
    private function career(string $saveId, array &$session): array
    {
        ...
        $careerContext = is_array($summary['career_context'] ?? null) ? $summary['career_context'] : [];
        $journeyRows = '';
        foreach ((array) ($careerContext['club_journey'] ?? []) as $clubChapter) {
            if (!is_array($clubChapter)) { continue; }
            $journeyRows .= '<li><strong>' . WebView::e($clubChapter['club_name'] ?? 'Club') . '</strong> · ' . WebView::e(implode(', ', array_map('strval', (array) ($clubChapter['seasons'] ?? [])))) . ' · ' . (int) ($clubChapter['appearances'] ?? 0) . ' apps · ' . (int) ($clubChapter['goals'] ?? 0) . ' goals · ' . (int) ($clubChapter['assists'] ?? 0) . ' assists</li>';
        }
        $journeyBody = $journeyRows === '' ? WebView::emptyState('No Club journey recorded yet.') : '<ul class="timeline">' . $journeyRows . '</ul>';
        $breakthrough = is_array($careerContext['breakthrough_club'] ?? null) ? $careerContext['breakthrough_club'] : null;
        $journeyNote = $breakthrough === null ? '' : '<p class="muted">Breakthrough Club: ' . WebView::e($breakthrough['club_name'] ?? 'Recorded Club') . '</p>';
        ...
        $body .= '<div class="dashboard-grid"><div class="dashboard-main">' . WebView::section('CURRENT SEASON', $snapshot['summary']['current_season_label'] ?? 'Current Season', $this->seasonFacts($summary)) . WebView::section('CLUB JOURNEY', 'Chronological senior record', $journeyBody . $journeyNote) . WebView::section('SEASON HISTORY', 'Record', $history) . WebView::section('DOMESTIC CUP HISTORY', 'Knockout record', $cupHistory) . WebView::section('EUROPEAN HISTORY', 'Continental record', $europeHistory) . '</div><aside class="dashboard-side">' . WebView::section('MOVEMENT HISTORY', 'Clubs', $movement) . WebView::section('POSITION HISTORY', 'Permanent football evolution', $positionHistory) . WebView::section('OFF-PITCH LIFE', 'Milestones', $life) . WebView::section('FOOTBALL SOCIAL HISTORY', 'Public milestones', $socialHistory) . WebView::section('PROFILE', 'Appearance', WebView::link('appearance', ['save' => $saveId], 'Customize appearance', 'button button-secondary')) . '</aside></div>';
    ...
    private function legacy(string $saveId, array &$session): array
    {
        ...
        $traits = is_array($legacy['traits'] ?? null) ? $legacy['traits'] : [];
        ...
        $clubJourney = (array) ($legacy['club_journey'] ?? []);
        ...
        if ($clubJourney !== []) {
            $clubBody .= '<ul class="timeline">' . implode('', array_map(static fn (array $club): string => '<li><strong>' . WebView::e($club['club_name'] ?? 'Club') . '</strong> · ' . (int) ($club['season_count'] ?? 0) . ' Seasons · ' . (int) ($club['appearances'] ?? 0) . ' apps</li>', $clubJourney)) . '</ul>';
        }
        if (is_array($legacy['breakthrough_club'] ?? null)) { $clubBody .= '<p class="muted">Breakthrough Club: ' . WebView::e($legacy['breakthrough_club']['club_name'] ?? 'Recorded Club') . '</p>'; }
        if (($legacy['one_club_career'] ?? false) === true) { $clubBody .= '<p class="result-badge">One-Club Career</p>'; }
        foreach ((array) ($legacy['returns'] ?? []) as $return) { if (is_array($return)) { $clubBody .= '<p class="muted">Return to ' . WebView::e($return['club_name'] ?? 'former Club') . ' · ' . WebView::e($return['season'] ?? '') . '</p>'; } }
        ...
        $timeline = '';
        foreach ((array) ($legacy['career_timeline'] ?? []) as $item) {
            if (is_array($item)) { $timeline .= '<li><strong>' . WebView::e($item['date'] ?? 'Recorded') . '</strong> · ' . WebView::e($item['title'] ?? 'Career landmark') . '</li>'; }
        }
        $timelineBody = $timeline === '' ? WebView::emptyState('No dated landmark evidence is available yet.') : '<ul class="timeline">' . $timeline . '</ul>';
        $comebackLines = '';
        foreach ((array) ($legacy['injury_comebacks'] ?? []) as $episode) {
            if (!is_array($episode)) { continue; }
            $injury = is_array($episode['injury'] ?? null) ? $episode['injury'] : [];
            $line = CareerLabels::value($episode['significance'] ?? null, 'Recorded injury') . ' · ' . ($injury['start_date'] ?? 'Recorded') . ' → ' . ($episode['medical_end_date'] ?? $injury['recovery_date'] ?? 'recovery');
            $return = is_array($episode['first_match_back'] ?? null) ? $episode['first_match_back'] : null;
            if ($return !== null) { $line .= ' · first Match back ' . ($return['date'] ?? 'Recorded'); }
            $comebackLines .= '<li>' . WebView::e($line) . '</li>';
        }
        $comebackBody = $comebackLines === '' ? WebView::emptyState('No significant injury episode with retained comeback evidence.') : '<ul class="timeline">' . $comebackLines . '</ul>';
        $defining = '';
        foreach ((array) ($legacy['defining_seasons'] ?? []) as $season) {
            if (is_array($season)) { $defining .= '<li><strong>' . WebView::e($season['season_id'] ?? 'Season') . '</strong> · ' . WebView::e(implode(' · ', array_map('strval', (array) ($season['reasons'] ?? [])))) . '</li>'; }
        }
    ...
    private function profile(string $saveId, string $playerId, array &$session): array
    {
        ...
        $onPitchRole = is_array($data['on_pitch_role'] ?? null) ? $data['on_pitch_role'] : [];
        $traits = is_array($data['traits'] ?? null) ? $data['traits'] : [];
        $establishedTraits = (array) ($traits['established'] ?? []);
        $emergingTraits = (array) ($traits['emerging'] ?? []);
        $traitRows = static function (array $items): string {
            return implode('', array_map(static fn (array $trait): string => '<li><strong>' . WebView::e($trait['label'] ?? 'Football identity') . '</strong> · ' . WebView::e($trait['description'] ?? '') . '</li>', $items));
        };
        $playingStyle = $establishedTraits === [] && $emergingTraits === []
            ? WebView::emptyState('No clear playing identity has emerged from the available evidence yet.')
        ...
        $direction = is_array($careerContext['direction'] ?? null) ? $careerContext['direction'] : [];
        $journeyLines = '';
        foreach (array_slice((array) ($careerContext['club_journey'] ?? []), 0, 6) as $chapter) {
            if (is_array($chapter)) {
                $journeyLines .= '<li><strong>' . WebView::e($chapter['club_name'] ?? 'Club') . '</strong> · ' . WebView::e(implode(', ', array_map('strval', (array) ($chapter['seasons'] ?? [])))) . ' · ' . (int) ($chapter['appearances'] ?? 0) . ' apps</li>';
            }
        }
        ...
        $recoveryPanel = '';
        if (($data['controlled'] ?? false) === true && ($recovery['visible'] ?? false) === true) {
            $return = is_array($recovery['first_match_back'] ?? null) ? $recovery['first_match_back'] : null;
            $recoveryText = '<div class="stat-grid compact">' . WebView::stat('Phase', CareerLabels::value($recovery['phase'] ?? null)) . WebView::stat('Significance', CareerLabels::value($recovery['significance'] ?? null)) . WebView::stat('Medical end', $recovery['medical_end_date'] ?? 'Recorded') . '</div><p>' . WebView::e($recovery['message'] ?? 'Recovery context is based on the recorded availability state.') . '</p>';
            if ($return !== null) {
                $recoveryText .= '<p><strong>First Match back:</strong> ' . WebView::e($return['date'] ?? 'Recorded') . ' · ' . WebView::e(implode(', ', array_map('strval', (array) ($return['performance'] ?? [])))) . '</p>';
            }
            $recoveryPanel = WebView::section('RETURN TO PLAY', 'Recorded recovery context', $recoveryText);
        }
        $disciplinePanel = '';
        if (($data['controlled'] ?? false) === true && ($discipline['active'] ?? false) === true) {
            $lines = '';
            foreach ((array) ($discipline['suspensions'] ?? []) as $suspension) {
                if (is_array($suspension)) { $lines .= '<li><strong>' . WebView::e($suspension['scope_label'] ?? 'Competition') . '</strong> · ' . WebView::e($suspension['reason_label'] ?? 'Disciplinary suspension') . ' · ' . (int) ($suspension['remaining'] ?? 0) . ' applicable Match' . ((int) ($suspension['remaining'] ?? 0) === 1 ? '' : 'es') . ' remaining</li>'; }
            }
            $disciplinePanel = WebView::section('DISCIPLINARY ELIGIBILITY', 'Current competition sanction', '<p class="result-badge">SUSPENDED</p><ul class="fixture-list">' . $lines . '</ul><p class="muted">Suspension changes eligibility only; it does not change readiness, injury, attributes or rating.</p>');
        }
        $cupPanel = '';
        ...
        $roleOptions = '';
        foreach ((array) ($onPitchRole['available_roles'] ?? []) as $roleOption) {
            if (!is_array($roleOption)) { continue; }
            $checked = ($roleOption['key'] ?? null) === ($onPitchRole['role'] ?? null) ? ' checked' : '';
            $roleOptions .= '<label class="choice-card"><input type="radio" name="role" value="' . WebView::e((string) ($roleOption['key'] ?? '')) . '"' . $checked . ' required><span><strong>' . WebView::e($roleOption['label'] ?? 'Role') . '</strong> · ' . WebView::e(CareerLabels::value($roleOption['suitability'] ?? null, 'Developing')) . '<br><small>' . WebView::e($roleOption['description'] ?? '') . ' ' . WebView::e($roleOption['why_it_fits'] ?? '') . '</small></span></label>';
        }
        ...
        if (($data['controlled'] ?? false) === true) {
            ...
            $landmarkLines = '';
            foreach (array_slice((array) ($legacy['career_landmarks'] ?? []), 0, 3) as $landmark) {
                if (is_array($landmark)) { $landmarkLines .= '<li><strong>' . WebView::e($landmark['title'] ?? 'Career landmark') . '</strong> · ' . WebView::e($landmark['date'] ?? 'Recorded') . '</li>'; }
            }
            $bestLines = '';
            foreach (array_slice((array) ($legacy['personal_bests'] ?? []), 0, 3) as $best) {
                if (is_array($best)) { $bestLines .= '<li>' . WebView::e($best['label'] ?? 'Personal best') . ' · ' . WebView::e($best['value'] ?? 'Recorded') . '</li>'; }
            }
            $legacyPanel = WebView::section('CAREER LEGACY', 'Historical achievement', '<div class="stat-grid compact">' . WebView::stat('Honours', count((array) ($legacy['honours'] ?? []))) . WebView::stat('Awards', count((array) ($legacy['awards'] ?? []))) . WebView::stat('Milestones', count((array) ($legacy['milestones'] ?? []))) . '</div>' . ($landmarkLines === '' ? '' : '<p><strong>Career landmarks</strong></p><ul class="fixture-list">' . $landmarkLines . '</ul>') . ($bestLines === '' ? '' : '<p><strong>Personal bests</strong></p><ul class="fixture-list">' . $bestLines . '</ul>') . WebView::link('legacy', ['save' => $saveId], 'Open Career Legacy', 'button button-secondary'));
        }
        $history = '';
    ...
    private function decision(string $saveId, array &$session): array
    {
        ...
        foreach ((array) ($decision['options'] ?? []) as $index => $option) {
            ...
            $details = '';
            if (($option['club_level'] ?? null) !== null) { $details .= ' · ' . (string) $option['club_level']; }
        ...
        $directionContext = is_array($currentContext['direction'] ?? null) ? $currentContext['direction'] : [];
        $currentContextFacts = '';
        if (($currentContext['role'] ?? null) !== null) { $currentContextFacts .= ' · Role: ' . CareerLabels::value($currentContext['role']); }
        if (($currentContext['playing_time'] ?? null) !== null) { $currentContextFacts .= ' · Playing time: ' . CareerLabels::value($currentContext['playing_time']); }
        if (($currentContext['club_objective'] ?? null) !== null) { $currentContextFacts .= ' · Objective: ' . CareerLabels::value($currentContext['club_objective']); }
    ...
    private function matchday(string $saveId, string $matchId, array &$session): array
    {
        ...
        $landmarkLines = '';
        foreach ((array) ($view['landmark_callouts'] ?? []) as $landmark) {
            if (is_array($landmark)) { $landmarkLines .= '<li><strong>' . WebView::e($landmark['title'] ?? 'Career landmark') . '</strong> · ' . WebView::e($landmark['description'] ?? 'Recorded from this Match.') . '</li>'; }
        }
