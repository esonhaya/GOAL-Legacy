...
final class PlayerMatchStatRepository
{
    ...
    { SchemaInitializationGuard::run($this->database->connection(), self::class, function (): void { $connection = $this->database->connection(); $connection->exec('CREATE TABLE IF NOT EXISTS ' . self::TABLE . ' (match_id TEXT NOT NULL, player_id TEXT NOT NULL, club_id TEXT NOT NULL, appeared INTEGER NOT NULL, started INTEGER NOT NULL, minutes INTEGER NOT NULL, goals INTEGER NOT NULL, assists INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (match_id, player_id))'); $columns = $connection->query('PRAGMA table_info(' . self::TABLE . ')')->fetchAll(PDO::FETCH_ASSOC); if (!array_filter($columns, static fn (array $column): bool => ($column['name'] ?? null) === 'assists')) { $connection->exec('ALTER TABLE ' . self::TABLE . ' ADD COLUMN assists INTEGER NOT NULL DEFAULT 0'); } $connection->exec('CREATE INDEX IF NOT EXISTS idx_match_stats_player ON ' . self::TABLE . ' (player_id, match_id)'); $connection->exec('CREATE INDEX IF NOT EXISTS idx_match_stats_club ON ' . self::TABLE . ' (club_id, match_id)'); }); }
    ...
    /** @return array<string, array{club_id:string,appearances:int,starts:int,minutes:int,goals:int,assists:int}> */
    public function seasonAggregates(SeasonId $seasonId): array
    {
        $statement = $this->database->connection()->prepare(
            'SELECT stats.player_id, stats.club_id, SUM(stats.appeared) AS appearances, SUM(stats.started) AS starts, SUM(stats.minutes) AS minutes, SUM(stats.goals) AS goals, SUM(stats.assists) AS assists '
    ...
    {
        $statement = $this->database->connection()->prepare(
            'SELECT stats.club_id, SUM(stats.appeared) AS appearances, SUM(stats.started) AS starts, SUM(stats.minutes) AS minutes, SUM(stats.goals) AS goals, SUM(stats.assists) AS assists '
            . 'FROM ' . self::TABLE . ' stats JOIN match_records matches ON matches.id = stats.match_id '
    ...
    private function hydrateRows(array $rows): array { return array_map(static fn (array $row): PlayerMatchStat => new PlayerMatchStat(new MatchId((string) $row['match_id']), new PlayerId((string) $row['player_id']), new ClubId((string) $row['club_id']), (bool) $row['appeared'], (bool) $row['started'], (int) $row['minutes'], (int) $row['goals'], (int) ($row['assists'] ?? 0)), $rows); }
}
