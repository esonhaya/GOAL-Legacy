...
final class PlayerMatchStatRepository
{
    ...
    public function seasonAggregates(SeasonId $seasonId): array
    {
        $statement = $this->database->connection()->prepare(
            'SELECT stats.player_id, stats.club_id, SUM(stats.appeared) AS appearances, SUM(stats.started) AS starts, SUM(stats.minutes) AS minutes, SUM(stats.goals) AS goals, SUM(stats.assists) AS assists, SUM(stats.shots) AS shots, SUM(stats.shots_on_target) AS shots_on_target, SUM(stats.saves) AS saves, SUM(stats.clean_sheets) AS clean_sheets, SUM(stats.tackles) AS tackles, SUM(stats.interceptions) AS interceptions, SUM(stats.blocks) AS blocks, SUM(stats.passes_attempted) AS passes_attempted, SUM(stats.passes_completed) AS passes_completed '
        ...
        foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            $aggregates[$playerId] = [
                ...
                'passes_attempted' => (int) $row['passes_attempted'],
    ...
    {
        $statement = $this->database->connection()->prepare(
            'SELECT stats.club_id, SUM(stats.appeared) AS appearances, SUM(stats.started) AS starts, SUM(stats.minutes) AS minutes, SUM(stats.goals) AS goals, SUM(stats.assists) AS assists, SUM(stats.shots) AS shots, SUM(stats.shots_on_target) AS shots_on_target, SUM(stats.saves) AS saves, SUM(stats.clean_sheets) AS clean_sheets, SUM(stats.tackles) AS tackles, SUM(stats.interceptions) AS interceptions, SUM(stats.blocks) AS blocks, SUM(stats.passes_attempted) AS passes_attempted, SUM(stats.passes_completed) AS passes_completed '
            . 'FROM ' . self::TABLE . ' stats JOIN match_records matches ON matches.id = stats.match_id '
