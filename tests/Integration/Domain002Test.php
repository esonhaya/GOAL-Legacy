...
final class Domain002Test extends TestCase
{
    ...
    public function testWorldNationCompetitionSeasonSaveAdvanceReloadPath(): void
    {
        ...
        self::assertCount(5, array_filter($services->competitionModule()->service()->repository($database)->all(), static fn ($competition): bool => $competition->status()->value === 'completed'));
        ...
        self::assertCount(5, array_filter($events, static fn (string $event): bool => $event === WorldEventNames::COMPETITION_ACTIVATED));
        self::assertCount(5, array_filter($events, static fn (string $event): bool => $event === WorldEventNames::COMPETITION_COMPLETED));
    }
}
