...
final class Domain006Test extends TestCase
{
    ...
    public function testCareerPlayerMatchLoopPersistsResultStatsHighlightsStandingsAndReload(): void
    {
        ...
        $careerSummary = null; foreach ($completed as $match) { $careerSummary = $matchService->playerSummary($database, $match->id(), $player->id()); if ($careerSummary !== null) { break; } } self::assertNotNull($careerSummary); self::assertTrue($careerSummary['appeared']); self::assertSame(90, $careerSummary['minutes']); self::assertSame('central_midfielder', $careerSummary['on_pitch_role']); self::assertSame('Central Midfielder', $careerSummary['on_pitch_role_label']);
